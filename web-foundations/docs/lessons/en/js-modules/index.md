...
- ❌ **Not native to browsers** - Needs bundlers (Browserify, Webpack)
...
**Why ES6 Modules Won:**
- ✅ **Native browser support** - No bundler needed (with caveats)
...
Create an `index.js` to aggregate exports:
...
export default function HomeView(container) {
  ...
  const header = createEl('header', {}, [
    createEl('h2', {}, ['Welcome to Modular JavaScript'])
  ]);
...
function createNav() {
  ...
  nav.appendChild(
    PrimaryButton('Home', () => navigate('home'))
  );
  nav.appendChild(
    SecondaryButton('About', () => navigate('about'))
  );
...
**⚠️ Important:** ES6 modules **require** a server due to CORS. You cannot use `file://` protocol.
...
export function isEmail(str) { /* ... */ }
export function fetchAPI(url) { /* ... */ } // Different concern!
export function formatCurrency(num) { /* ... */ } // Different concern!
```
...
**🎨 Atelier Philosophy:**
*"We don't teach code—we teach thinking. Modules are not syntax, they are architecture. Architecture is not structure, it is culture. Culture is how we work together across time and space. Welcome to the community of vernacular web masters."*
